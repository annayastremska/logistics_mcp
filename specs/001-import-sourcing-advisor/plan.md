...
**Testing**: `pytest` — 23 tests over the domain arithmetic and contract behaviour, offline, 1.4 s.
...
| **V. Verified, not asserted** (non-negotiable) | **PASS** | Every claim in the artefacts is backed by a run recorded in this session: full agent run, failure-path run, offline run, browser render, 23 tests. The end-to-end check asserts on substance, not on completion. |
...
tests/                       # 23 offline tests over the arithmetic and contract behaviour
